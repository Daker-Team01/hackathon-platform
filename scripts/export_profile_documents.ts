...
import {
  buildHackathons,
  buildTeamProfileDocument,
  buildUserProfileDocument,
  createHackathonMap,
  type RawHackathonProfileSource,
  type RawTeamProfileSource,
  type RawUserProfileSource,
} from '../src/lib/profileDocuments.js'
...
const loadEnvFile = async (envPath: string) => {
  try {
...
const loadJsonArray = async <T>(filePath: string): Promise<T[]> => {
...
const mapSupabaseTeamToRawTeam = (row: SupabaseTeamRow): RawTeamProfileSource => {
  return {
...
const fetchTeamsFromSupabase = async (): Promise<RawTeamProfileSource[]> => {
  ...
  return ((data ?? []) as SupabaseTeamRow[]).map((row) => mapSupabaseTeamToRawTeam(row))
}

async function main() {
  const [rawUsers, rawHackathons, rawTeams] = await Promise.all([
    loadJsonArray<RawUserProfileSource>(defaultUserPath),
    loadJsonArray<RawHackathonProfileSource>(defaultHackathonPath),
