...
function recommendForUser(uid, matrix, normalizedRows, itemIds) {
    const sims = normalizedRows.map((row, candidateIdx) => candidateIdx === uid ? 0 : cosineSimilarity(normalizedRows[uid], row));
    const neighbors = sims
        .map((sim, idx) => ({ sim, idx }))
        .sort((a, b) => b.sim - a.sim)
        .slice(0, TOPK_USER)
        .filter((item) => item.sim > 0);
    const seen = new Set(matrix[uid]
